...
use crate::host::{nybl_to_sim_error, NyblCodeHost};
...
pub fn run_simulation(code: &str, puzzle: &PuzzleConfig) -> SimulationResult {
    ...
    // Parse (nybl lib handles lex+parse)
    let stmts = match nybl::parse(code) {
        Ok(stmts) => stmts,
        Err(e) => return error_result(nybl_to_sim_error(e)),
    };

    // Run with NyblCodeHost
    let mut host = NyblCodeHost {
    ...
    let nybl_limits = NyblLimits {
        max_steps: limits.max_steps,
        max_memory: limits.max_memory,
    };

    let run_error = nybl_vm::run(code, &mut host, &nybl_limits).err();
    ...
    // - Otherwise, use the nybl runtime error (if any)
    let error = if host.halt_error.is_some() {
        ...
        run_error.map(nybl_to_sim_error)
    };
    ...
    let instructions = nybl::count_instructions(&stmts);
